...
#     df = pd.read_pickle('manifest.pickle')
#     genes = df.UCSC_RefGene_Name
#     chrs = df.CHR
#     chr_locations = df.MAPINFO
#     cg_ids = df.IlmnID
#     gene_regions = df.UCSC_RefGene_Name
#     master_dict = {}
...
#     return master_dict, exons, exon_ids

# read in csv data file
def read_csv(filename):
    tp = pd.read_csv(filename, sep='\t', usecols=[3, 7, 8], iterator=True, chunksize=1000, low_memory=False)
...
#                 gene_dict2[gene] = 0
#             gene_dict2[gene] += 1
#             if beta_value >= beta_value_cutoff and cg in exon_ids:
#                 gene_methylated += 1
...
#          'https://dcc.icgc.org/api/v1/download?fn=/current/Projects/KIRC-US/meth_array.KIRC-US.tsv.gz', 'https://dcc.icgc.org/api/v1/download?fn=/current/Projects/COAD-US/meth_array.COAD-US.tsv.gz' ]
...
filenames = ['download?fn=%2Fcurrent%2FProjects%2FBLCA-US%2Fmeth_array.BLCA-US.tsv', 'download?fn=%2Fcurrent%2FProjects%2FBRCA-US%2Fmeth_array.BRCA-US.tsv', 'download?fn=%2Fcurrent%2FProjects%2FCESC-US%2Fmeth_array.CESC-US.tsv',
          ...
          'download?fn=%2Fcurrent%2FProjects%2FHNSC-US%2Fmeth_array.HNSC-US.tsv', 'download?fn=%2Fcurrent%2FProjects%2FKIRC-US%2Fmeth_array.KIRC-US.tsv', 'download?fn=%2Fcurrent%2FProjects%2FKIRP-US%2Fmeth_array.KIRP-US.tsv',
          'download?fn=%2Fcurrent%2FProjects%2FLAML-US%2Fmeth_array.LAML-US.tsv', 'download?fn=%2Fcurrent%2FProjects%2FLGG-US%2Fmeth_array.LGG-US.tsv',  'download?fn=%2Fcurrent%2FProjects%2FLIHC-US%2Fmeth_array.LIHC-US.tsv',
...
# # files = ['https://dcc.icgc.org/api/v1/download?fn=/current/Projects/PAEN-AU/meth_array.PAEN-AU.tsv.gz']
# total_methylated = 0
...
# master_dict, exons, exon_ids = create_master_dict(manifest_location)
...
# sample_count = [] #list of number of samples for each cancer type
# exon_count = {}
for f in filenames:
    # try:
...
#     print('Number of methylated sites in exons per sample for ' + f[len(f) - 14:-7] + ': %d' % (geneMethylated / numSamples))
