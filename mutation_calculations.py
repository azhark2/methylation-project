...
for file in ssms:
	with open(file.split('.')[2] + '_mutation_calculations.tsv', 'w') as csvout:
    	...
    	for sample in df['id'].unique():
    		...
    		a = pybedtools.BedTool(sample + '_all_CpG_mutations.bed')
    		a.intersect('cds.bed').saveas(sample + '_coding_CpG_mutations.bed') #file of all CpG mutations in coding region for that sample
    		remove_duplicates.remove(sample + '_coding_CpG_mutations.bed')
    	    os.rename(sample + '_coding_CpG_mutations.bed' + '.noDuplicates', sample + '_coding_CpG_mutations.bed')
