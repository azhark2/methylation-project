...
# create dictionary of cg_id --> gene name
# create dictionary of cg_id --> chromosome, chromosome coordinate
def create_master_dict(manifest_location):
    ...
    pd.read_pickle('manifest.pickle')
...
def read_csv(filename):
    tp = pd.read_csv(filename, sep='\t', usecols=[7, 8], iterator=True, chunksize=1000, low_memory=False, compression = 'gzip')
...
def count_total_methylated_sites():
    methylated = df.loc[df.iloc[:, 1] >= 0.8]
    ...
    for cg in cgs:
        if cg in master_dict:
            ...
            if gene not in gene_dict:
                gene_dict[gene] = 0
            gene_dict[gene] += 1
    return sum(gene_dict.values())


#main

# filenames = ['https://dcc.icgc.org/api/v1/download?fn=/current/Projects/BLCA-US/meth_array.BLCA-US.tsv.gz', 'https://dcc.icgc.org/api/v1/download?fn=/current/Projects/BRCA-US/meth_array.BRCA-US.tsv.gz',
...
all_beta_values = []
filenames = ['https://dcc.icgc.org/api/v1/download?fn=/current/Projects/PAAD-US/meth_array.PAAD-US.tsv.gz']
for f in filenames:
    try:
        t0 = time.time()
        ...
        print ('It took %d seconds' %(total_time))
    except:
        t0 = time.time()
        df = read_csv(f)
        t1 = time.time()
        total_time = t1-t0
        df.to_pickle(f[len(f) - 14:-6] + 'pickle')
    ...
    print('Number of methylated sites in coding regions for ' + f[len(f) - 14:-7] + ': %d' % (geneMethylated))
...
plt.ylabel("Frequency")
plt.xticks(np.arange(0, 1.1, 0.1))
plt.savefig('all_beta_values.png')
plt.show()
...
sites = [pair[1] for pair in sorted_genes]
print (genes[:10])
print (sites[:10])
...
plt.barh(np.arange(20), sites[:20], align='center')
plt.yticks(np.arange(20), genes)
plt.show()
...
gene_dict = pickle.load( open( "/Users/khandekara2/Documents/methylationProject/02_code/gene_dict.pickle ", "rb" ))
...
f = 'https://dcc.icgc.org/api/v1/download?fn=/current/Projects/BRCA-US/meth_array.BRCA-US.tsv.gz'
print (f[len(f) - 14:-10])


# In[45]:

sitecount = {'a':(5, 6), 'b': (6, 7)}
sitecount['a'][0]
sitecount['a'][1]
...
def count_gene_methylated_sites():
    methylated = df.loc[df.iloc[:, 1] >= 0.8]
    cgs = list(methylated.iloc[:, 0])
    for cg in cgs: #check if probe id is in master_dict
        if cg in master_dict:
            gene_tuple = master_dict[cg]
            gene = gene_tuple[0].split(';')
            gene = gene[0]
            if gene not in gene_dict:
                gene_dict[gene] = 0
            gene_dict[gene] += 1
    allSites = df.loc[df.iloc[:, 1]]
    cgs = list(allSites.iloc[:, 0])
    for cg in cgs: #check if probe id is in master_dict
        if cg in master_dict:
            gene_tuple = master_dict[cg]
            gene = gene_tuple[0].split(';')
...
print ('Finished with manifest')
all_beta_values = []
filenames = ['https://dcc.icgc.org/api/v1/download?fn=/current/Projects/PAEN-AU/meth_array.PAEN-AU.tsv.gz']
...
genes = [pair[0] for pair in sorted_genes]
number_of_sites = [pair[1] for pair in sorted_genes]


for gene, count in zip(sorted_genes, number_of_sites):
...
plt.barh(np.arange(20), number_of_sites[:20], align='center')
plt.yticks(np.arange(20), genes)
...
plt.yticks(np.arange(20), topCosmicGenes)
plt.savefig('top_methylated_genes.png')
...
plt.yticks(np.arange(20), cancer_types)
...
plt.barh(np.arange(len(cancer_types)), gene_sites, align='center')
